 ...
 * @return Non-zero value in case of an error,prints to stdout the maximum grade
 * * if the grade is out of range we print a message to stderr
 */
int main(int argc, char **argv) {
	...
	int grade,maxgrade,rerror=fscanf(fp," %d", &maxgrade);
	if (rerror !=1){
      		/* Error */
   ...
   while (1){
     	...
     	if (grade < MIN_GRADE || grade > MAX_GRADE) {
		  ...
		  maxgrade=(maxgrade<grade)? grade:maxgrade;
		  num_lines++;
	  }
